...
const HandlerPushEvents = async function (events) {
  ...
  const gitlabTimestamp = dayjs().format('YYYY-MM-DD HH:mm:ss') // events.commits[0].timestamp
...
const HandlerMergeEvents = async function (events) {
  ...
  const gitlabTimestamp = dayjs().format('YYYY-MM-DD HH:mm:ss') //events.object_attributes.created_at
